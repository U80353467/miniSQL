...
BPlusTree::BPlusTree(index_id_t index_id, BufferPoolManager *buffer_pool_manager, const KeyManager &KM,
    ...
    if (leaf_max_size_ == UNDEFINED_SIZE) {
      leaf_max_size_ = static_cast<int>((PAGE_SIZE - LEAF_PAGE_HEADER_SIZE) / (KM.GetKeySize() + sizeof(RowId)) - 1);
    }
    if (internal_max_size_ == UNDEFINED_SIZE) {
      internal_max_size_ = static_cast<int>((PAGE_SIZE - INTERNAL_PAGE_HEADER_SIZE) / (KM.GetKeySize() + sizeof(page_id_t)));
    }
...
bool BPlusTree::GetValue(const GenericKey *key, std::vector<RowId> &result, Transaction *transaction) {
  ...
  auto leaf = reinterpret_cast<LeafPage *>(FindLeafPage(key, root_page_id_, false)->GetData());
  int index = leaf->KeyIndex(key, processor_);
  if (index < leaf->GetSize() && processor_.CompareKeys(leaf->KeyAt(index), key) == 0) {
...
bool BPlusTree::InsertIntoLeaf(GenericKey *key, const RowId &value, Transaction *transaction) {
  ...
  if (processor_.CompareKeys(new_leaf->KeyAt(0), key) <= 0) {
...
BPlusTreeInternalPage *BPlusTree::Split(InternalPage *node, Transaction *transaction) {
  ...
  auto new_node = reinterpret_cast<InternalPage *>(buffer_pool_manager_->NewPage(new_page_id)->GetData());
  if (new_node == nullptr) {
    throw runtime_error("out of memory");
  }
...
BPlusTreeLeafPage *BPlusTree::Split(LeafPage *node, Transaction *transaction) {
  page_id_t new_page_id = INVALID_PAGE_ID;
  auto new_node = reinterpret_cast<LeafPage *>(buffer_pool_manager_->NewPage(new_page_id)->GetData());
  if (new_node == nullptr) {
    throw runtime_error("out of memory");
  }
...
void BPlusTree::InsertIntoParent(BPlusTreePage *old_node, GenericKey *key, BPlusTreePage *new_node,
  ...
  if (parent_page_id == INVALID_PAGE_ID) {
    auto parent_page = reinterpret_cast<InternalPage *>(buffer_pool_manager_->NewPage(parent_page_id)->GetData());
    root_page_id_ = parent_page_id;
    if (parent_page == nullptr) {
      throw runtime_error("out of memory");
    }
    parent_page->Init(parent_page_id, INVALID_PAGE_ID, processor_.GetKeySize(), internal_max_size_);
...
bool BPlusTree::CoalesceOrRedistribute(N *&node, Transaction *transaction) {
  ...
  if (sibling_page->GetSize() + node->GetSize() <= node->GetMaxSize()) {
    bool is_parent_being_deleted = false;
    if (is_sibling_right) {
      is_parent_being_deleted = Coalesce(node, sibling_page, parent_page, index + 1, transaction);
    } else {
      is_parent_being_deleted = Coalesce(sibling_page, node, parent_page, index, transaction);
    }
    if (is_parent_being_deleted) {
...
bool BPlusTree::Coalesce(InternalPage *&neighbor_node, InternalPage *&node, InternalPage *&parent, int index,
  ...
  auto leaf = reinterpret_cast<LeafPage *>(FindLeafPage(middle_key, node->ValueAt(0), true)->GetData());
  middle_key = leaf->KeyAt(0);
  buffer_pool_manager_->UnpinPage(leaf->GetPageId(), false);
...
bool BPlusTree::AdjustRoot(BPlusTreePage *old_root_node) {
  if (old_root_node->GetSize() == 0) {
    ...
    auto new_root_node = reinterpret_cast<InternalPage *>(buffer_pool_manager_->FetchPage(new_root_page_id)->GetData());
...
IndexIterator BPlusTree::Begin() {
  auto leaf = reinterpret_cast<LeafPage *>(FindLeafPage(nullptr, root_page_id_, true)->GetData());
...
Page *BPlusTree::FindLeafPage(const GenericKey *key, page_id_t page_id, bool leftMost) {
  ...
  BPlusTreePage *tree_node = reinterpret_cast<BPlusTreePage *>(page->GetData());
  if (tree_node->IsLeafPage()) {
    return page;
  }
  auto internal_node = reinterpret_cast<InternalPage *>(tree_node);
  page_id_t next_level_page_id;
  if (leftMost) {
    next_level_page_id = internal_node->ValueAt(0);
  } else {
    next_level_page_id = internal_node->Lookup(key, processor_);
  }
  Page *result = FindLeafPage(key, next_level_page_id, leftMost);
  buffer_pool_manager_->UnpinPage(page_id, false);
  return result;
}
...
void BPlusTree::ToGraph(BPlusTreePage *page, BufferPoolManager *bpm, std::ofstream &out) const {
  ...
  if (page->IsLeafPage()) {
    ...
    for (int i = 0; i < leaf->GetSize(); i++) {
      out << "<TD>" << leaf->KeyAt(i) << " " << leaf->ValueAt(i).GetPageId() << " " << leaf->ValueAt(i).GetSlotNum() << "</TD>\n";
    }
    out << "</TR>";
    // Print table end
    out << "</TABLE>>];\n";
