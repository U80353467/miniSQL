...
dberr_t BPlusTreeIndex::ScanKey(const Row &key, const Row &key_, vector<RowId> &result, Transaction *txn, string compare_operator) {
  GenericKey *index_key = processor_.InitKey();
  processor_.SerializeFromKey(index_key, key, key_schema_);
  GenericKey *index_key_ = processor_.InitKey();
  processor_.SerializeFromKey(index_key_, key_, key_schema_);
  if (compare_operator == "r") {
    for (auto iter = GetBeginIterator(index_key); iter != GetBeginIterator(index_key_); ++iter) {
      result.emplace_back((*iter).second);
    }
  }
  delete index_key;
